...
- [`contract import`](#contract-import) - Import smart contract ABI
...
```bash
l8b contract import <address> --chain <chain> --name <name> [options]
```

### Options

- `address` - Contract address (required)
- `--chain <chain>` - Blockchain network: `base`, `ethereum`, `optimism`, or `arbitrum` (required)
- `--name <name>` - Contract name for the generated wrapper (required)
- `--api-key <key>` - Block explorer API key (optional, can also set `BASE_API_KEY`, `ETHEREUM_API_KEY`, etc.)
- `--root <path>` - Path to project root (default: current directory)

### Examples

```bash
# Import contract from Base network
l8b contract import 0x123... --chain base --name MyContract

# Import with custom API key
l8b contract import 0x123... --chain ethereum --name MyContract --api-key YOUR_KEY

# Import from specific project
l8b contract import 0x123... --chain base --name MyContract --root ./my-game
```

### Environment Variables for API Keys

You can set API keys as environment variables instead of using `--api-key`:

```bash
export BASE_API_KEY=your_key_here
export ETHEREUM_API_KEY=your_key_here
export OPTIMISM_API_KEY=your_key_here
export ARBITRUM_API_KEY=your_key_here
```
