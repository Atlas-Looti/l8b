...
events.on("boss_appeared", function()
    tween.create({
        from = 1, to = 1.5, duration = 1000, easing = "easeInOutQuad",
        onUpdate = function(v)
            zoom_t.v = v
...
function enter_room(rx, ry)
    -- Snap camera to room center with a short tween
    local target_x = rx * ROOM_W + ROOM_W / 2
    local target_y = ry * ROOM_H + ROOM_H / 2
    local fake = { x = camera.getX(cam), y = camera.getY(cam) }
    camera.unfollow(cam)
    tween.create({
        from = fake.x, to = target_x, duration = 300, easing = "easeInOutCubic",
        onUpdate = function(v)
            fake.x = v
...
local function draw_pixel_perfect()
    camera.begin(cam)
