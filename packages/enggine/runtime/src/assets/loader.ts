...
export class AssetLoader {
					...
					new Promise<void>(
						(
							...
							try {
								...
								this.collections.sprites[
									name
								] =
									sprite;
							} catch (err) {
								console.error(
									`Failed to load sprite ${name}:`,
									err,
								);
	...
	private countReady(): number {
		...
		for (const sound of Object.values(
			this
				.collections
				.sounds,
		)) {
		...
		for (const mus of Object.values(
			this
				.collections
				.music,
		)) {
	...
	showLoadingBar(
		...
		const progress =
			this.getProgress();
		screenInterface.clear(
			"#000",
		);
		screenInterface.drawRect(
			0,
			0,
			100,
			10,
