...
interface L8BVMContext {
	...
	warnings: {
		using_undefined_variable: Record<
			string,
			any
		>;
		assigning_field_to_undefined: Record<
			string,
			any
		>;
		invoking_non_function: Record<
			string,
			any
		>;
		assigning_api_variable: Record<
			string,
			any
		>;
		assignment_as_condition: Record<
			string,
			any
		>;
	};
...
export class Thread {
	...
	addCall(
		...
		if (
			this.next_calls.indexOf(
				call,
			) < 0
		) {
			this.next_calls.push(
				call,
			);
		}
	}
...
export class Runner {
	...
	init(): boolean {
		...
		this.system.profiler =
			{
				...
				getMetrics:
					() =>
						this.profiler.getAverageMetrics(),
			};

		this.l8bvm.context.global.print =
			this.l8bvm.context.meta.print;
		this.l8bvm.context.global.random =
			new Random(
				0,
			);
		...
		this.l8bvm.context.global.Math =
			MathLib;
		this.l8bvm.context.global.JSON =
			JSONLib;
				...
				switch (
					...
					case "assigning_api_variable":
						if (
							...
							this.l8bvm.context.warnings.assigning_api_variable[
								id
							] =
								{
									file:
					...
					case "assignment_as_condition":
						if (
							...
							this.l8bvm.context.warnings.assignment_as_condition[
								id
							] =
								{
									file:
		...
		if (
			...
			if (
				...
				if (
					...
					f =
						this.main_thread.processor.routineAsFunction(
							routine,
							this
								.l8bvm
								.context,
	...
	sleep(
		...
		if (
			...
			this.current_thread.sleep_until =
				Date.now() +
				Math.max(
					0,
					value,
