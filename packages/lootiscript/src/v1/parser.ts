...
export class Parser {
		...
		try {
				...
				if (
					...
					if (
						...
						if (
							...
							this.error(
								`Misuse of reserved keyword: '${token.value}'`,
							);
						}
					} else {
						this.error(
			...
			if (
				...
				let functionName:
					...
					| null =
					null;
				let functionStartLine:
					| number
					| null =
					null;
				let functionStartColumn:
					| number
					| null =
					null;
	...
	parseLine(): Statement | null {
		...
		switch (
			...
			case Token.TYPE_LOCAL:
				return this.parseLocalAssignment(
					token,
				);
			default:
	...
	parseExpressionSuffix(
		...
		switch (
			...
			case Token.TYPE_PLUS_EQUALS:
				return this.parseSelfAssignment(
					token,
					expression,
					token.type,
				);
	...
	parseBinaryOperation(
		...
		return Program.BuildOperations(
			ops,
			terms,
		);
	}
	...
	parseArrowFunction(
		...
		for (const arg of args) {
			if (
				...
				throw this.error(
					"Invalid argument in arrow function",
				);
			}
		}
	...
	addTerminable(
		...
		return this.not_terminated.push(
			token,
		);
	}
			...
			while (
				...
				if (
					...
					if (
						...
						this.error(
							"Unexpected data while parsing function",
						);
					}
				}
			}
	...
	parseIf(
		...
		this.warningAssignmentCondition(
			current.condition,
		);
		token =
			this.nextToken();
		...
		while (
			...
			if (
				...
				if (
					...
					throw Error(
						"Unexpected data while parsing if",
					);
				}
				line =
					parsedLine;
		...
		while (
			...
			if (
				...
				if (
					...
					this.error(
						"Unexpected data while parsing after",
					);
				}
			}
		}
