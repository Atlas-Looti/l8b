...
export class Compiler {
	...
	constructor(
		...
		this.count +=
			this.routine.opcodes.length;
		this.routine.locals_size =
			this.locals.max_index;
	}
		...
		while (
			...
			if (
				ops[
					...
					OPCODES.LOAD_VARIABLE &&
				ops[
					i + 1
				] ===
					OPCODES.FUNCTION_CALL
			) {
				const varName =
					args[
						i
	...
	compileAssignment(
		...
		if (
			...
			if (
				statement.field instanceof
				Variable
			) {
				if (
					statement.expression instanceof
					Function
				) {
					index =
						this.locals.register(
							statement
								.field
								.identifier,
						); // register function locally first
					this.compile(
						statement.expression,
					); // then compile function which may refer to itself
					this.routine.arg1[
						this
							.routine
							.arg1
					...
					index =
						this.locals.register(
							statement
								.field
								.identifier,
						); // register thread locally first
					arg_index =
						this
							.routine
							.arg1
							.length; // thread main routine will land here
					this.compile(
						statement.expression,
					); // then compile function which may refer to itself
					this.routine.arg1[
					...
					this.compile(
						statement.expression,
					); // first compile expression which may refer to another local with same name
					index =
						this.locals.register(
							statement
								.field
								.identifier,
						); // then register a local for that name
					this.routine.STORE_LOCAL(
						index,
						statement,
					);
			...
			if (
				...
				f =
					statement.field as Field;
				if (
					f.expression instanceof
					Variable
				) {
					if (
						f
						...
						this.routine.LOAD_LOCAL_OBJECT(
							index,
							f.expression,
						);
					} else if (
				...
				for (
					...
					this.routine.LOAD_PROPERTY_OBJECT(
						f
							.chain[
							i
						],
	...
	compileSelfAssignment(
		statement: SelfAssignment,
	): void {
		let f: Field,
			i: number,
			index: number,
		...
		switch (
			...
			default:
				throw (
					"Unknown self-assignment operation: " +
					statement.operation
				);
		}
		if (
			...
			this.routine.LOAD_PROPERTY_ATOP(
				statement,
			);
			this.compile(
				statement.expression,
		...
		if (
			...
			switch (
				...
				case "%":
					...
					break;
				case "&":
					this.routine.BINARY_AND(
						op,
					);
					break;
				case "|":
					this.routine.BINARY_OR(
						op,
					);
					break;
				case "<<":
					this.routine.SHIFT_LEFT(
						op,
					);
					break;
				case ">>":
					this.routine.SHIFT_RIGHT(
						op,
					);
			}
		} else if (
	...
	compileFunctionCall(
		...
		if (
			...
			this.compileFieldParent(
				call.expression as Field,
			);
			this.compile(
				(
					call.expression as Field
			...
			this.routine.FUNCTION_APPLY_PROPERTY(
				call
					.args
					.length,
				call,
			...
			if (
				...
				this.routine.FUNCTION_APPLY_VARIABLE(
					call
						.args
						.length,
					call,
	...
	compileFor(
		...
		if (
			forloop.range_from instanceof
				Value &&
			forloop
				.range_from
				.type ===
				Value.TYPE_NUMBER &&
			forloop.range_to instanceof
				Value &&
			forloop
				.range_to
				.type ===
	...
	compileFunction(
		...
		var r: Routine;
		r =
			this.compileFunctionBody(
				func,
			);
		this.routine.LOAD_ROUTINE(
			r,
			func,
		);
	}
	...
	compileCreateClass(
		...
		variable =
			statement.ext !=
				null &&
			statement.ext instanceof
				Variable
				? statement
						.ext
						.identifier
				: 0;
		this.routine.CREATE_CLASS(
			variable,
			statement,
		);
		ref =
	...
	compileNewCall(
		...
		let a: Statement,
			...
			ref: Statement[];
		call =
			statement.expression as FunctionCall;
		this.routine.LOAD_VALUE(
			0,
			statement,
		); // reserve spot on stack for the class instance
		ref =
			call.args;
		for (
			i = j =
		...
		this.routine.POP(
			statement,
		); // pop return value of class constructor
	}
	...
	static predefined_unary_functions: Record<
		string,
		(
			x: any,
		) => number
	...
	static predefined_values: Record<
		string,
		number
	> = {
		PI: Math.PI,
