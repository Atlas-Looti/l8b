...
export const syntaxMessages: Record<
	string,
	MessageTemplate
> = {
	...
	[SyntaxErrorCode.E1003]:
		{
			...
			description:
				"A block was opened but not closed with 'end'",
			suggestions:
				[
	...
	[SyntaxErrorCode.E1005]:
		{
			...
			message:
				(
					...
					`Missing ${args.expected || "token"}`,
			description:
				"A required token was expected but not found",
			suggestions:
				[
	...
	[SyntaxErrorCode.E1006]:
		{
			...
			description:
				"The file ended while parsing was incomplete",
			suggestions:
				[
	...
	[SyntaxErrorCode.E1007]:
		{
			...
			message:
				(
					...
					`Misuse of reserved keyword '${args.keyword}'`,
			description:
	...
	[SyntaxErrorCode.E1009]:
		{
			...
			message:
				(
					...
					`Unterminated ${args.type || "object/array"}`,
			description:
				"An object or array was started but not closed",
			suggestions:
				[
