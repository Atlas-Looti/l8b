...
export const drawMessages: Record<
	string,
	MessageTemplate
> = {
	[APIErrorCode.E7091]:
		{
			...
			description:
				"A drawing operation could not be completed",
			suggestions:
				[
	...
	[APIErrorCode.E7092]:
		{
			...
			category:
				DiagnosticCategory.API,
			message:
				"Invalid drawing context",
			description:
				"The drawing context is invalid or not available",
			suggestions:
				[
	...
	[APIErrorCode.E7093]:
		{
			...
			message:
				(
					...
					`Invalid drawing parameters: ${args.error || "unknown"}`,
			description:
				"Drawing parameters are invalid",
			suggestions:
				[
