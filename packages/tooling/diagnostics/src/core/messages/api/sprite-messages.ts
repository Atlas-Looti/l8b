...
export const spriteMessages: Record<
	string,
	MessageTemplate
> = {
	[APIErrorCode.E7021]:
		{
			...
			message:
				(
					...
					`Sprite loading failed: ${args.url || "unknown URL"}`,
			description:
				"The sprite image could not be loaded",
			suggestions:
				[
	...
	[APIErrorCode.E7022]:
		{
			...
			message:
				(
					...
					`Invalid sprite properties: ${args.error || "unknown"}`,
			description:
				"Sprite properties are invalid",
			suggestions:
				[
	...
	[APIErrorCode.E7023]:
		{
			...
			message:
				(
					...
					`Invalid sprite URL: ${args.url}`,
			description:
				"The sprite URL is invalid or malformed",
			suggestions:
				[
