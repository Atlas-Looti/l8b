...
export const assetMessages: Record<
	string,
	MessageTemplate
> = {
	[APIErrorCode.E7041]:
		{
			...
			message:
				(
					...
					`Asset not found: '${args.assetName}'`,
			description:
	...
	[APIErrorCode.E7042]:
		{
			...
			message:
				(
					...
					`Asset loading failed: ${args.assetName || "unknown"}`,
			description:
	...
	[APIErrorCode.E7043]:
		{
			...
			message:
				(
					...
					`Invalid asset type: '${args.assetType}'`,
			description:
	...
	[APIErrorCode.E7044]:
		{
			...
			message:
				(
					...
					`Asset '${args.assetName}' is not ready`,
			description:
				"The asset exists but has not finished loading",
			suggestions:
				[
