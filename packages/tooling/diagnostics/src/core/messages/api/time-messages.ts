...
export const timeMessages: Record<
	string,
	MessageTemplate
> = {
	[APIErrorCode.E7081]:
		{
			...
			message:
				(
					...
					`Invalid time value: ${args.value || "unknown"}`,
			description:
				"The time value is invalid or out of range",
			suggestions:
				[
	...
	[APIErrorCode.E7082]:
		{
			...
			message:
				(
					...
					`Time playback failed: ${args.error || "unknown"}`,
			description:
				"Time playback operation could not be completed",
			suggestions:
				[
	...
	[APIErrorCode.E7083]:
		{
			...
			message:
				(
					...
					`Time recording failed: ${args.error || "unknown"}`,
			description:
				"Time recording operation could not be completed",
			suggestions:
				[
	...
	[APIErrorCode.E7084]:
		{
			...
			message:
				(
					...
					`Invalid time format: ${args.format || "unknown"}`,
			description:
				"The time format is not supported",
			suggestions:
				[
