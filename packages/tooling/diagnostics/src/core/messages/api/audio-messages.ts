...
export const audioMessages: Record<
	string,
	MessageTemplate
> = {
	[APIErrorCode.E7011]:
		{
			...
			description:
				"The browser could not create an AudioContext",
			suggestions:
				[
	...
	[APIErrorCode.E7012]:
		{
			...
			category:
				DiagnosticCategory.API,
			message:
				"Audio worklet failed to start",
			description:
				"The audio worklet processor could not be initialized",
			suggestions:
				[
	...
	[APIErrorCode.E7013]:
		{
			...
			message:
				(
					...
					`Sound not found: '${args.soundName}'`,
			description:
				"The requested sound does not exist in the sound collection",
			suggestions:
				[
	...
	[APIErrorCode.E7014]:
		{
			...
			message:
				(
					...
					`Music not found: '${args.musicName}'`,
			description:
				"The requested music does not exist in the music collection",
			suggestions:
				[
	...
	[APIErrorCode.E7015]:
		{
			...
			description:
				"The audio context requires user interaction to resume",
			suggestions:
				[
	...
	[APIErrorCode.E7016]:
		{
			...
			message:
				(
					...
					`Invalid audio parameters: ${args.error || "unknown"}`,
			description:
				"Audio playback parameters are invalid",
			suggestions:
				[
