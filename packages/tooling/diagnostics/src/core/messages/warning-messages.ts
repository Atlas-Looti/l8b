	...
	[WarningCode.W1001]:
		{
			...
			message:
				(
					...
					`Assigning to API variable '${args.variableName}'`,
			description:
				"You are modifying a read-only API variable",
			suggestions:
				[
	...
	[WarningCode.W2001]:
		{
			...
			message:
				(
					...
					`Deprecated API usage: ${args.apiName}`,
			description:
				"A deprecated API is being used",
			suggestions:
				[
	...
	[WarningCode.W5001]:
		{
			...
			message:
				(
					...
					`Activating first available scene: ${args.sceneName}`,
			description:
				"No route matched, so the first scene was activated",
			suggestions:
				[
	...
	[WarningCode.W5002]:
		{
			...
			description:
				"The initial path didn't match any route",
			suggestions:
				[
					"Register a route for the initial path",
	...
	[WarningCode.W5003]:
		{
			...
			category:
				DiagnosticCategory.Scene,
			message:
				"No scenes registered. Game may show blank screen.",
			description:
				"No scenes have been registered",
			suggestions:
				[
	...
	[WarningCode.W5004]:
		{
			...
			description:
				"Router was initialized before any scenes were registered",
			suggestions:
				[
