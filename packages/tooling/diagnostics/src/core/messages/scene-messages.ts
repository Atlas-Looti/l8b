...
export const sceneMessages: Record<
	string,
	MessageTemplate
> = {
	[SceneErrorCode.E5001]:
		{
			...
			description:
				"An invalid scene path was provided",
			suggestions:
				[
	...
	[SceneErrorCode.E5002]:
		{
			...
			message:
				(
					...
					`Invalid scene name: ${args.name}`,
			description:
				"An invalid scene name was provided",
			suggestions:
				[
	...
	[SceneErrorCode.E5003]:
		{
			...
			message:
				(
					...
					`Invalid scene definition for '${args.name}'`,
			description:
				"A scene was defined with invalid configuration",
			suggestions:
				[
	...
	[SceneErrorCode.E5004]:
		{
			...
			message:
				(
					...
					`Scene not found: ${args.name}`,
			description:
				"A requested scene could not be found",
			suggestions:
				[
	...
	[SceneErrorCode.E5005]:
		{
			...
			message:
				(
					...
					`No route matched for path: ${args.path}`,
			description:
				"No scene route matched the provided path",
			suggestions:
				[
	...
	[SceneErrorCode.E5006]:
		{
			...
			description:
				"No scenes have been registered in the router",
			suggestions:
				[
	...
	[SceneErrorCode.E5007]:
		{
			...
			message:
				(
					...
					`Scene '${args.name}' has no draw() function`,
			description:
				"A scene is missing the required draw function",
			suggestions:
				[
