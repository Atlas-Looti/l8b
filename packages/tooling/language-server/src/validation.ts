...
import {
	API_ACCESS_REGEX,
	GLOBAL_API,
} from "./api-definitions/index";
import type {
	DocumentRegionsCache,
	LanguageModes,
} from "./embedded/mode-manager";
...
export async function validateTextDocument(
	...
	const documentRegions =
		documentRegionsCache.get(
			textDocument,
		);
	const allModes =
		languageModes.getAllModes();
...
function validateApiUsage(
	...
	while (
		...
		const fullPath =
			match[1]; // e.g., "screen", "sprites.player", "map.level1"
		const propertyName =
			match[2]; // e.g., "drawSprite", "invalidProperty"
