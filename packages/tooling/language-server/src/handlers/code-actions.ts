...
export function setupCodeActionsHandler(connection: Connection, documents: TextDocuments<TextDocument>) {
	connection.onCodeAction((params: CodeActionParams): CodeAction[] => {
		try {
			...
			connection.console.error(`Code action error: ${error instanceof Error ? error.message : String(error)}`);
			return [];
		}
	});
}
