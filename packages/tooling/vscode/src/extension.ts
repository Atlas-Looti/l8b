...
export function activate(
	...
	console.log(
		"LootiScript Language Server extension is activating...",
	);
	...
	statusBarItem.tooltip =
		"LootiScript Language Server: Starting...";
	statusBarItem.command =
		"workbench.actions.view.problems"; // Open problems on click
	statusBarItem.show();
	...
	client
		...
		.then(
			async () => {
				statusBarItem.text =
					"$(pass) L8B";
				statusBarItem.tooltip =
					"LootiScript Language Server: Running";
				console.log(
					"LootiScript Language Server started successfully",
				);
				await hydrateApiReference(
					apiProvider,
				);
			},
		)
		.catch(
			(
				...
				apiProvider.setError(
					"Unable to load API reference",
				);
				console.error(
					"Failed to start LootiScript Language Server:",
					error,
				);
			},
		);
	...
	console.log(
		"LootiScript Language Server extension is now active!",
	);
}
		...
		vscode.commands.registerCommand(
			...
			async () => {
				...
				statusBarItem.text =
					"$(sync~spin) L8B";
				statusBarItem.tooltip =
					"LootiScript Language Server: Restarting...";
...
function updateStatusBarWithDiagnostics() {
	...
	if (
		...
		statusBarItem.text =
			"$(pass) L8B";
		statusBarItem.tooltip =
			"LootiScript Language Server: No problems";
		statusBarItem.backgroundColor =
			undefined;
	}
}
...
function createEnhancedHover(
	...
	if (
		...
		if (
			suggestionContent.value.includes(
				"💡",
			)
		) {
			contents.push(
...
async function hydrateApiReference(
	apiProvider: ApiProvider,
): Promise<void> {
	if (
		!client
	...
	try {
		const apiData =
			await client.sendRequest<GlobalApiMap>(
				GLOBAL_API_REQUEST,
			);
		apiProvider.setApiData(
			apiData,
		);
	} catch (error) {
		apiProvider.setError(
		...
		console.error(
			"Failed to load LootiScript API reference",
			error,
		);
	}
}
