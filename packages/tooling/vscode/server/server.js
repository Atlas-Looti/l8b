...
var LanguageModes = class {
	...
	getModeAtPosition(
		...
		const regions =
			this.documentRegions.get(
				document,
			);
		const languageId =
			regions.getLanguageAtPosition(
				position,
			);
		if (
			languageId
	...
	return {
		...
		async doComplete(
			...
			const documentRegions =
				documentRegionsCache2.get(
					document,
				);
			const embedded =
				documentRegions.getEmbeddedDocument(
					"json",
				);
			const jsonDocument =
				jsonLanguageService2.parseJSONDocument(
					embedded,
				);
			const completions =
				await jsonLanguageService2.doComplete(
					embedded,
					position,
					jsonDocument,
				);
		...
		async doHover(
			...
			return await jsonLanguageService2.doHover(
				embedded,
				position,
				jsonDocument,
			);
		...
		async doValidation(
			...
			return await jsonLanguageService2.doValidation(
				embedded,
				jsonDocument,
				{
					schemaValidation:
		...
		findDocumentSymbols(
			...
			return jsonLanguageService2.findDocumentSymbols2(
				embedded,
				jsonDocument,
			);
		},
...
function createJSONLanguageService() {
	return (0,
	import_vscode_json_languageservice.getLanguageService)(
		{
...
var coreApi =
	{
					...
					lastIndexOf:
						{
							type:
								"method",
							signature:
								"List.lastIndexOf(arr, item, from?)",
							description:
		...
		Math: {
			type:
				"module",
			description:
				"Mathematical and game-specific utilities",
			properties:
				{
					abs: {
						type:
						...
						slice:
							{
								...
								description:
									"Extract substring (supports negative indices)",
							},
						indexOf:
							{
								type:
									"method",
								signature:
									"String.indexOf(str, search, from?)",
								description:
						...
						lastIndexOf:
							{
								type:
									"method",
								signature:
									"String.lastIndexOf(str, search, from?)",
								description:
						...
						padStart:
							{
								type:
									"method",
								signature:
									"String.padStart(str, length, pad?)",
								description:
									"Pad string at start",
							},
						padEnd:
		...
		ObjectPool:
			{
				...
				signature:
					"new ObjectPool(factory, maxSize?)",
				properties:
					{
						acquire:
							{
		...
		storage:
			{
				type:
					"module",
				description:
					"Persistent storage API (localStorage wrapper)",
				properties:
					{
						get: {
							type:
		...
		screen:
			{
				type:
					"object",
				description:
					"Screen drawing and display interface",
				properties:
					{
						width:
							{
						...
						clearScreen:
							{
								...
								signature:
									"screen.clearScreen(color?: string)",
							},
						drawCircle:
							{
								type:
						...
						clear:
							{
								...
								signature:
									"screen.clear(color?)",
								description:
									"Clear screen (alias for clearScreen)",
							},
						setColor:
							{
								type:
						...
						setPixelated:
							{
								...
								description:
									"Enable/disable pixelated rendering",
							},
						setBlending:
							{
								type:
						...
						setDrawAnchor:
							{
								type:
									"method",
								signature:
									"screen.setDrawAnchor(ax, ay)",
								description:
									"Set sprite drawing anchor",
							},
						setDrawRotation:
							{
								type:
						...
						drawRoundRect:
							{
								type:
									"method",
								signature:
									"screen.drawRoundRect(x, y, w, h, r?, color?)",
								description:
						...
						drawRound:
							{
								type:
									"method",
								signature:
									"screen.drawRound(x, y, w, h, color?)",
								description:
									"Draw round outline",
							},
						drawImage:
							{
								type:
									"method",
								signature:
									"screen.drawImage(sprite, x, y, w?, h?)",
								description:
									"Draw image (alias for drawSprite)",
							},
						drawSpritePart:
							{
								type:
						...
						drawImagePart:
							{
								type:
									"method",
								signature:
									"screen.drawImagePart(sprite, sx, sy, sw, sh, x, y, w?, h?)",
								description:
									"Draw part of image",
							},
						drawMap:
...
var audioApi =
	{
		audio: {
			type:
				"object",
			description:
				"Audio playback and sound interface",
			properties:
				{
					beep:
						{
							...
							description:
								"Play a beep sound",
							signature:
								"audio.beep(frequency?: number, duration?: number)",
						},
					playSound:
						{
							type:
					...
					stopSound:
						{
							...
							signature:
								"audio.stopSound(soundName: string)",
						},
					setVolume:
						{
							type:
...
var inputApi =
	{
		input: {
			type:
				"object",
			description:
				"User input interface (keyboard, mouse, touch, gamepad)",
			properties:
				{
					keyboard:
						{
					...
					touch:
						{
							type:
								"property",
							description:
								"Touch input state for mobile devices",
						},
					gamepad:
						{
							type:
		...
		system:
			{
				type:
					"object",
				description:
					"Runtime system utilities and information",
				properties:
					{
						time:
							{
...
var sceneApi =
	{
		scene: {
			...
			signature:
				"scene(name: string, definition: object)",
		},
		route: {
			type:
				"function",
			description:
				"Register a route to a scene",
			signature:
						...
						replace:
							{
								type:
									"method",
								description:
									"Replace current path (no history)",
								signature:
						...
						goto:
							{
								type:
									"method",
								description:
									"Directly switch to a scene",
								signature:
									"scenes.goto(name: string, params?: object)",
							},
						current:
							{
								type:
		...
		loadSprite:
			{
				type:
					"function",
				description:
					"Load a sprite from source",
				signature:
					"loadSprite(src, options?)",
			},
		updateSprite:
			{
				type:
		...
		player:
			{
				type:
					"object",
				description:
					"Farcaster player context and user information",
				properties:
					{
						fid: {
							type:
						...
						getPfpUrl:
							{
								...
								signature:
									"player.getPfpUrl()",
								description:
									"Get player profile picture URL",
							},
						getContext:
							{
								type:
								...
								signature:
									"player.getContext()",
								description:
									"Get full player context object",
							},
						isInMiniApp:
							{
								type:
		...
		wallet:
			{
				type:
					"object",
				description:
					"Wallet operations for Farcaster Mini Apps",
				properties:
					{
						isConnected:
							{
...
var evmApi =
	{
		evm: {
			type:
				"object",
			description:
				"EVM blockchain operations using viem",
			properties:
				{
					read:
						{
							...
							description:
								"Read from smart contract (view function, no transaction)",
						},
					write:
						{
							type:
					...
					call:
						{
							...
							description:
								"Call/simulate contract function (no transaction)",
						},
					getBalance:
						{
							type:
					...
					formatEther:
						{
							type:
								"method",
							signature:
								"evm.formatEther(value: string)",
							description:
								"Format wei to ether (wei / 10^18)",
						},
					parseEther:
						{
							type:
								"method",
							signature:
								"evm.parseEther(value: string)",
							description:
								"Parse ether to wei (ether * 10^18)",
						},
				},
		},
	};
		...
		actions:
			{
				type:
					"object",
				description:
					"Farcaster SDK actions for Mini Apps",
				properties:
					{
						ready:
							{
								type:
									"method",
								signature:
									"actions.ready(disableNativeGestures?: boolean)",
								description:
						...
						share:
							{
								type:
									"method",
								signature:
									"actions.share({text?: string, embeds?: string[]})",
								description:
						...
						openMiniApp:
							{
								type:
									"method",
								signature:
									"actions.openMiniApp({url: string})",
								description:
						...
						viewProfile:
							{
								type:
									"method",
								signature:
									"actions.viewProfile({fid: number})",
								description:
						...
						viewCast:
							{
								type:
									"method",
								signature:
									"actions.viewCast({hash: string, close?: boolean})",
								description:
						...
						viewToken:
							{
								type:
									"method",
								signature:
									"actions.viewToken({token: string})",
								description:
									"View a token",
							},
						composeCast:
...
function deleteDocumentState(
	uri,
) {
	documentStates.delete(
		uri,
	);
}
__name(
		...
		document.getText(
			{
				...
				end: import_node.Position.create(
					position.line +
						1,
					0,
				),
...
function getWordAtPosition2(
	...
	const line =
		document.getText(
			{
				start:
					import_node2.Position.create(
						position.line,
						0,
					),
				end: import_node2.Position.create(
					position.line,
...
async function getDocumentSettings(
	...
	const configuration =
		await connection2.workspace.getConfiguration(
			{
				scopeUri:
					resource,
				section:
	...
	try {
		...
		if (
			...
			const diagnosticData =
				(0,
				import_diagnostics.createDiagnostic)(
					err.code ||
						"E1004",
					{
						file:
			...
			const lspDiagnostic =
				(0,
				import_diagnostics.formatForLSP)(
					diagnosticData,
				);
			diagnostics.push(
				lspDiagnostic,
...
function setupCompletionHandlers(
	...
	connection2.onCompletion(
		async (
			...
			const position =
				params.position;
			const mode =
				languageModes2.getModeAtPosition(
					document,
					position,
				);
			if (
	...
	connection2.onCompletionResolve(
		(
			item,
		) => {
			const info =
...
function setupSignatureHelpHandler(
	connection2,
	documents2,
	hasConfigurationCapability2,
) {
	connection2.onSignatureHelp(
		async (
			params,
		) => {
			const document =
			...
			const line =
				document.getText(
					{
						start:
							import_node5.Position.create(
								params
									.position
									.line,
								0,
							),
						end: import_node5.Position.create(
							params
								.position
								.line,
							params
			...
			return {
				signatures:
					[
						import_node5.SignatureInformation.create(
							signature,
							description,
							...parameters,
						),
	...
	connection2.onDefinition(
		(
			...
			if (
				...
				return [];
			return import_node6.Location.create(
				symbol.documentUri,
				symbol.range,
			);
		},
	);
	connection2.onDocumentSymbol(
		(
			params,
		) => {
			const documentStates2 =
...
function setupFormattingHandler(
	connection2,
	documents2,
	hasConfigurationCapability2,
) {
	connection2.onDocumentFormatting(
		async (
			params,
		) => {
			const doc =
			...
			const formatted =
				lines.map(
					(
						...
						if (
							/(then|do|function|repeat)\b.*$/.test(
								trimmed,
							) &&
							!trimmed.includes(
								"end",
...
__name(
	setupFormattingHandler,
	"setupFormattingHandler",
);

// src/handlers/code-actions.ts
var import_node8 = require("vscode-languageserver/node");
function setupCodeActionsHandler(
	connection2,
	documents2,
) {
	connection2.onCodeAction(
		(
			params,
		) => {
			const actions =
				[];
			for (const diagnostic of params
				.context
				.diagnostics) {
				const message =
		...
		(
			...
			for (const symbol of state.symbols) {
				if (
					...
					return import_node9.Location.create(
						params
							.textDocument
							.uri,
						symbol.range,
...
function setupReferencesHandler(
	connection2,
	documents2,
) {
	connection2.onReferences(
		...
		(
			...
			const line =
				document.getText(
					{
						start:
							import_node11.Position.create(
								params
									.position
									.line,
								0,
							),
						end: import_node11.Position.create(
							params
								.position
								.line +
								1,
...
function setupSemanticTokensHandler(
	connection2,
	documents2,
) {
	connection2.languages.semanticTokens.on(
		(
			params,
		) => {
			const document =
			...
			const builder =
				new import_node12.SemanticTokensBuilder();
			const text =
				document.getText();
			const lines =
				text.split(
...
var connection =
	(0,
	import_node13.createConnection)(
		import_node13
			.ProposedFeatures
			.all,
	);
var documents =
	new import_node13.TextDocuments(
		import_vscode_languageserver_textdocument2.TextDocument,
	);
var documentRegionsCache =
	new DocumentRegionsCache();
var languageModes =
	new LanguageModes(
		documentRegionsCache,
	);
var jsonLanguageService =
	createJSONLanguageService();
...
var GLOBAL_API_REQUEST =
	"lootiscript/globalApi";
