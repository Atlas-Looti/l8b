...
function formatCompileErrorOutput(
	error: CompileError,
): string {
	if (
		error.diagnostic
...
function formatCompileWarningOutput(
...
export async function compileSources(
			...
			const diagnostic =
				createDiagnostic(
					CompilationErrorCode.E3002,
					{
						...
						suggestions:
							[
								`Ensure ${normalizedPath} exists and is readable.`,
							],
						data:
							{
								filePath:
		...
		if (
			...
			if (
				...
				for (const warning of result.warnings) {
					...
					console.warn(
						pc.yellow(
							`    ⚠ ${moduleName}: ${warning.warning ?? "Warning"}`,
						),
					);
					logDiagnosticLines(
						formatCompileWarningOutput(
							warning,
						),
						(
							line,
	...
	console.log(
		pc.green(
			`  ✓ Compiled ${compiled.length} modules`,
		),
	);
