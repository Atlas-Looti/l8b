						...
						[
							lootiScriptPlugin(),
							{
								...
								configureServer(
									...
									const normalizedDistFontPath =
										path.normalize(
											fontPaths.dist,
										);
									const normalizedAssetsFontPath =
										path.normalize(
											fontPaths.assets,
										);
										...
										async (
											...
											if (
												...
												try {
													...
													console.error(
														"Error generating OG image page:",
														error,
													);
													res.statusCode = 500;
													res.end(
		...
		if (
			...
			try {
				...
				if (
					...
					console.log(
						pc.gray(
							"   Tip: Install 'qrcode-terminal' for QR code generation\n",
						),
					);
				}
			} catch (error) {
