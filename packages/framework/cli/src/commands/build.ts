...
export async function build(
	...
	console.log(
		pc.green(
			`  ✓ Found ${Object.keys(sources).length} source files`,
		),
	);
	if (
		Object.keys(
	...
	console.log(
		pc.green(
			`  ✓ Saved ${compileResult.compiled.length} compiled modules`,
		),
	);
	...
	if (
		...
		console.log(
			pc.gray(
				"  Generating Farcaster manifest...",
			),
		);
		const manifestJson =
			generateFarcasterManifestJSON(
				config,
			);
		if (
			manifestJson
			...
			console.log(
				pc.green(
					"  ✓ Generated /.well-known/farcaster.json",
				),
			);
		}
	}
	...
	console.log(
		pc.green(
			`  ✓ Generated ${DEFAULT_FILES.INDEX_HTML}`,
		),
	);
	...
	console.log(
		pc.cyan(
			"  Run `l8b start` to preview the production build\n",
		),
	);
}
