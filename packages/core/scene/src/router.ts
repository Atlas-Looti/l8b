...
export class Router {
	 ...
	 */
	private handleNoRouteMatch(
		path: string,
	): void {
		if (
			!this.sceneManager.hasActiveScene()
		) {
			this.activateFirstAvailableScene(
				path,
			);
		}
	}
	 ...
	 */
	private activateFirstAvailableScene(
		path: string,
	): void {
